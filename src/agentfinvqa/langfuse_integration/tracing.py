...
try:
    ...
    @contextmanager
    def propagate_attributes(**_: object) -> Iterator[None]:  # type: ignore[misc]
...
class _TraceHandle:
    ...
    def score_trace(self, name: str, value: float) -> None:
        ...
        if self._span is not None:
            with contextlib.suppress(Exception):
                self._span.score_trace(name=name, value=value)  # type: ignore[attr-defined]
...
def sample_trace(
    ...
    with (
        ...
        trace_id = client.get_current_trace_id()  # type: ignore[attr-defined]
...
def close_span(
    ...
    with contextlib.suppress(Exception):
        ...
        if update_kwargs:
            span.update(**update_kwargs)  # type: ignore[attr-defined]
        ...
        if isinstance(value, (int, float)):
            with contextlib.suppress(Exception):
                if hasattr(trace, "score_trace"):
                    trace.score_trace(name=name, value=float(value))
